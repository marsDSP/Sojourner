...
namespace MarsDSP {
	...
	const int predelay = 16000;
	const int vlfpredelay = 16000;

	const int delayA = 466;		const int delayF = 719;		const int delayK = 787;
	const int delayB = 233;		const int delayG = 130;		const int delayL = 124;
	const int delayC = 747;		const int delayH = 511;		const int delayM = 72;
	const int delayD = 70;		const int delayI = 148;		const int delayN = 667;
	const int delayE = 776;		const int delayJ = 84;		const int delayO = 773;


	const int delayP = 29;		const int delayU = 11;
	const int delayQ = 710;		const int delayV = 73;
	const int delayR = 309;		const int delayW = 756;
	const int delayS = 773;		const int delayX = 627;
	const int delayT = 487;		const int delayY = 328;
    ...
    public:
    	...
    	void prepare(juce::dsp::ProcessSpec& spec)
    	{
    		...
    		for (int x = 0; x < pear_total; x++) {pearA[x] = 0.0;
    			pearB[x] = 0.0; pearC[x] = 0.0; pearD[x] = 0.0; pearE[x] = 0.0; pearF[x] = 0.0;}
    	...
    	void setParams(float in, float regen, float derez, float predly, float wet)
    	{
    	            ...
    	            MarsDSP::Smoothers& smoothers)
    	{
			...
			{
				float A = smoothers.getNextInput();
				float B = smoothers.getNextRegen();
				float C = smoothers.getNextDeRez();
				float D = smoothers.getNextPreDelay();
				float E = smoothers.getNextWetLevel();
                ...
                if (adjPredelay > predelay) adjPredelay = predelay;
    	...
    	void setInputGain() noexcept
    	{
    	}

    	void setRegen() noexcept
    	{
    	}

    	void getDeRez() noexcept
    	{
    	}

    	void getPreDelay() noexcept
    	{
    	}

    	void getWetLevel() noexcept
    	{
    	}
    	...
    	private:
    		...
    		double aAL[delayA+5];		double aFL[delayF+5];		double aKL[delayK+5];
    		double aBL[delayB+5];		double aGL[delayG+5];		double aLL[delayL+5];
    		double aCL[delayC+5];		double aHL[delayH+5];		double aML[delayM+5];
    		double aDL[delayD+5];		double aIL[delayI+5];		double aNL[delayN+5];
    		double aEL[delayE+5];		double aJL[delayJ+5];		double aOL[delayO+5];

    		double aPL[delayP+5];		double aUL[delayU+5];		double aAR[delayA+5];
    		double aQL[delayQ+5];		double aVL[delayV+5];		double aBR[delayB+5];
    		double aRL[delayR+5];		double aWL[delayW+5];		double aCR[delayC+5];
    		double aSL[delayS+5];		double aXL[delayX+5];		double aDR[delayD+5];
    		double aTL[delayT+5];		double aYL[delayY+5];		double aER[delayE+5];

    		double aFR[delayF+5];		double aKR[delayK+5];		double aPR[delayP+5];
    		double aGR[delayG+5];		double aLR[delayL+5];		double aQR[delayQ+5];
    		double aHR[delayH+5];		double aMR[delayM+5];		double aRR[delayR+5];
    		double aIR[delayI+5];		double aNR[delayN+5];		double aSR[delayS+5];
    		double aJR[delayJ+5];		double aOR[delayO+5];		double aTR[delayT+5];

    		double aUR[delayU+5];		double aZL[predelay+5];
    		double aVR[delayV+5];		double aZR[predelay+5];
    		double aWR[delayW+5];		double aVLFL[vlfpredelay+5];
    		double aXR[delayX+5];		double aVLFR[vlfpredelay+5];
    		double aYR[delayY+5];

    		double feedbackAL = 0.0;			double feedbackER = 0.0;			double feedblurAL = 0.0;		double feedblurER = 0.0;
    		double feedbackBL = 0.0;			double feedbackJR = 0.0;			double feedblurBL = 0.0;		double feedblurJR = 0.0;
    		double feedbackCL = 0.0;			double feedbackOR = 0.0;			double feedblurCL = 0.0;		double feedblurOR = 0.0;
    		double feedbackDL = 0.0;			double feedbackTR = 0.0;			double feedblurDL = 0.0;		double feedblurTR = 0.0;
    		double feedbackEL = 0.0;			double feedbackYR = 0.0;			double feedblurEL = 0.0;		double feedblurYR = 0.0;

    		double sbAL = 0.0;				double sbER = 0.0;
    		double sbBL = 0.0;				double sbJR = 0.0;
    		double sbCL = 0.0;				double sbOR = 0.0;
    		double sbDL = 0.0;				double sbTR = 0.0;
    		double sbEL = 0.0;				double sbYR = 0.0;
    		...
    		double pearA[pear_total] = {0.0};
    		double pearB[pear_total] = {0.0};
    		double pearC[pear_total] = {0.0};
    		double pearD[pear_total] = {0.0};
    		double pearE[pear_total] = {0.0};
    		double pearF[pear_total] = {0.0};
    		...
    		double previousAL = 0.0;
    		double previousBL = 0.0;
    		double previousCL = 0.0;
    		double previousDL = 0.0;
    		double previousEL = 0.0;

    		double iirAR = 0.0;
    		double iirBR = 0.0;

    		double previousAR = 0.0;
    		double previousBR = 0.0;
    		double previousCR = 0.0;
    		double previousDR = 0.0;
    		double previousER = 0.0;

    		int countAL = 1;		int countFL = 1;		int countKL = 1;		int countPL = 1;		int countUL = 1;
    		int countBL = 1;		int countGL = 1;		int countLL = 1;		int countQL = 1;		int countVL = 1;
    		int countCL = 1;		int countHL = 1;		int countML = 1;		int countRL = 1;		int countWL = 1;
    		int countDL = 1;		int countIL = 1;		int countNL = 1;		int countSL = 1;		int countXL = 1;
    		int countEL = 1;		int countJL = 1;		int countOL = 1;		int countTL = 1;		int countYL = 1;

    		int countAR = 1;		int countFR = 1;		int countKR = 1;		int countPR = 1;		int countUR = 1;
    		int countBR = 1;		int countGR = 1;		int countLR = 1;		int countQR = 1;		int countVR = 1;
    		int countCR = 1;		int countHR = 1;		int countMR = 1;		int countRR = 1;		int countWR = 1;
    		int countDR = 1;		int countIR = 1;		int countNR = 1;		int countSR = 1;		int countXR = 1;
    		int countER = 1;		int countJR = 1;		int countOR = 1;		int countTR = 1;		int countYR = 1;
